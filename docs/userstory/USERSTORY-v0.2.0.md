...
**인수 기준 (Given/When/Then)**
1. Given 감시 종목이 2개 이상이고 동시 모니터링 중일 때, When 한 종목이 가장 먼저 매수 조건을 만족하면, Then 시스템은 해당 종목에 사용 가능 자금을 전액 배정해 `현재가+2틱` 지정가 매수 주문을 낸다.
...
**인수 기준 (Given/When/Then)**
1. Given 모니터링 화면이 표시될 때, When 테이블 헤더를 확인하면, Then 아래 12개 칼럼이 정확히 이 순서로 존재한다: 종목명|종목코드|8시30분 가격|현재 가격|전저점 시간|전저점 가격|매수 시간|매수 가격|전고점 시간|전고점 가격|매도 시간|매도 가격.
...
- 매수 실행: 매수 조건을 가장 먼저 만족한 1개 종목에 `현재가+2틱` 지정가로 전액 매수
- 매도 트리거:
...
| I-18 | 20260217 | 08:30 기준가, -1% 하락 후 전저점 대비 +0.2% 반등 시 `현재가+2틱` 매수 | US-02, US-03, US-04 |
