...
|---|---|---|---|---|
| ILD-TSE-TC-001 | 08:30 이전 무동작 | 감시종목 1개, 루프 RUNNING | 08:29:59 시세 입력 | 상태 전이/BUY 없음 |
| ILD-TSE-TC-002 | 기준가 1회 확정 | 08:30 이후 첫 시세 도착 | 동일 종목 시세 2회 입력 | 첫 틱만 `base_price` 저장 |
| ILD-TSE-TC-003 | -1% 하락 후 +0.2% 반등 매수 | 게이트 OPEN, 포지션 없음 | 하락→전저점 갱신→반등 입력 | `BUY_SIGNAL` 1회, 상태 `BUY_REQUESTED`, BUY 주문가=`현재가+2틱` |
